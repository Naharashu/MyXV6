...
// Source - https://stackoverflow.com/a/3987783
// Posted by Benoit Thiery
...
void exit(void) {
    ...
    if (removeprocfile(curproc->pid) < 0)
        cprintf("cannot remove /proc/%d\n", curproc->pid);
...
void forkret(void) {
    ...
    safestrcpy(status, "Name:\t", sizeof(status));
    ...
    strcat(status, "Memory:\t");
    strcat(status, itoa(p->sz, 10));
    strcat(status, "\n");

    strcat(status, "Killed:\t");
    strcat(status, itoa(p->killed, 10));
    strcat(status, "\n");

    if (createprocfile(p->pid, status) < 0)
        cprintf("cannot create /proc/%d\n", p->pid);
